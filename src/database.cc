...
namespace muse{
	...
	void Database::ConnectDatabase(){
		...
		if(this->con == NULL){
			std::cerr << "Could not init the mysql" << std::endl;
			exit(EXIT_FAILURE);
		}
		...
		if(ret == NULL){
			std::cerr << "Error: " << mysql_error(this->con) << std::endl;
			exit(EXIT_FAILURE);
		}
...
//		cv::waitKey(0);
	...
	void Database::ExecuteInsertQuery(int id, cv::Mat descriptor){
		...
		if(statement == NULL){
			std::cerr << "Error: " << mysql_stmt_error(statement) << std::endl;
			exit(EXIT_FAILURE);
		}
	...
	cv::Mat *Database::LoadOnMemory(){
		...
		std::cout << "Feature information is loaded on memory successfully." << std::endl;
